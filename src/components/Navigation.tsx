...
export default function Navigation({ setCurrentTab, onOpenProduct, onOpenCart, cartCount = 0 }: any) {
  ...
  const NAV_LINKS = [
    ...
    { label: s.nav.specs, href: "#specs" },
    { label: s.nav.reviews, href: "#reviews" },
  ];
    ...
    <header className="bg-white">
      ...
      <div className="hidden md:block bg-[#fafafa] border-b border-line">
        <div className="max-w-mi mx-auto px-5 h-9 flex items-center justify-between text-xs text-muted">
          <p className="font-medium">{s.nav.partner}</p>
          <div className="flex items-center gap-5">
            <a
              href={`https://wa.me/573148145417?text=${encodeURIComponent(s.wa.support)}`}
              target="_blank"
            ...
            <button onClick={toggle} className="flex items-center gap-1.5 hover:text-ink transition-colors cursor-pointer" aria-label={s.nav.langAria}>
      ...
      <div className="border-b border-line">
        <div className="max-w-mi mx-auto px-5 h-16 md:h-[72px] flex items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={toggle}
              className="inline-flex items-center gap-1 min-h-11 px-2 text-sm font-semibold text-body hover:text-mi transition-colors cursor-pointer"
              aria-label={s.nav.langAria}
            >
            ...
            <button
              ...
              aria-label={cartCount > 0 ? s.nav.viewCartN(cartCount) : s.nav.viewCart}
            >
            ...
            <button
              ...
              aria-label={mobileMenuOpen ? s.nav.closeMenu : s.nav.openMenu}
            >
          ...
          <div className="px-5 py-3">
            ...
            {NAV_LINKS.map((link) => (
              <a
                ...
                href={link.href}
                onClick={goHome}
                className="block text-body hover:text-mi py-3 font-medium border-b border-line/60"
              >
                {link.label}
              </a>
            ))}
            <button
              ...
              className="flex items-center gap-2 w-full text-left text-body hover:text-mi py-3 font-medium border-b border-line/60"
            >
              <Globe className="w-4 h-4" strokeWidth={1.8} />
              {s.nav.region}
            </button>
